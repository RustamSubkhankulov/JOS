...
found:
    ...
    // cprintf("page2pa(new) + CLASS_MASK(new->class) %llx IOPHYSMEM %x \n", page2pa(new) + CLASS_MASK(new->class), IOPHYSMEM);
...
init_memory(void) {
    ...
    cprintf("Performing tests. Trying to allocate all pages. \n");

    int flags = 0;
    int class = 0; // allocating by smallest possible class pages

    // struct Page* prev     = NULL;
    // struct Page* prevprev = NULL;
    // struct Page* page = NULL;

    // prevprev = alloc_page(class, flags);
    // prev     = alloc_page(class, flags);
    ...
    // while(1)
    // {
    //     page = alloc_page(class, flags);

    //     if (page == NULL)
    //     {
    //         page_unref(prev);
    //         page_unref(prevprev);
    //         assert(page && "alloc_page() returned NULL, free prev");
    //     }
    //     else 
    //     {
    //         page_ref(page);
    ...
    struct Page* pages[5000] = { 0 };
    unsigned count = 0;

    pages[count] = alloc_page(class, flags);

    if (pages[count] != NULL)
    {
        page_ref(pages[count]);
        count++;
    }

    for (unsigned iter = 0; iter < count; iter++)
    {
        page_unref(pages[iter]);
    }
